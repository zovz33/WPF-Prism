...
namespace WPFPrismServiceApp.ViewModels.DialogViewModels
{
    public class AddDialogViewModel : BindableBase, IDialogAware
    {
        ...
        void ExecuteCloseDialogCommand(string parameter)
        {
                ...
                var dialogResult = new DialogResult(result, new DialogParameters { { "AddedUser", Fields } });
        ...
        private bool ValidateFields()
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(field.EditValueText) || field.EditValueText.Length < 3)
                {
                    EditErrorMessage = "Поля не должны быть пустыми и иметь более 3-ёх символов."; 
